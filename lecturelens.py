...
# ── API retry / rate-limit protection ─────────────────────────────────────
def is_rate_limit_error(err):
    """Detect common Claude/Gemini/HTTP rate-limit or quota errors."""
    msg = str(err).lower()
    return (
        "429" in msg
...
def call_with_retry(api_call, label="API", max_retries=6):
    """Run an API call immediately. Only waits/retries if a rate-limit-style error happens."""
    delays = [10, 20, 40, 60, 90, 120]
    ...
    for attempt in range(max_retries):
        try:
            ...
            if is_rate_limit_error(e) and attempt < max_retries - 1:
                ...
                print(f"  [!] {label} rate/usage limit hit. Waiting {wait_time}s, then retrying... ({attempt + 1}/{max_retries})")
                time.sleep(wait_time)
                continue
            raise


def claude_messages_create_with_retry(**kwargs):
    return call_with_retry(lambda: client.messages.create(**kwargs), label="Claude")


# ── Gemini hybrid settings ────────────────────────────────────────────────
# Normal camera captures use Gemini to reduce cost.
# Claude Sonnet is still used for Smart Lookback/delete, screenshots, notes, and final summary.
USE_GEMINI_FOR_CAMERA_CAPTURES = True
GEMINI_CAMERA_MODEL = "gemini-3.1-flash-lite"
# If your Google account does not have Gemini 3.1 Flash-Lite yet, change the line above to:
# GEMINI_CAMERA_MODEL = "gemini-2.5-flash-lite"


def load_gemini_api_key():
    """Load Gemini API key from config.txt, hardcoded fallback, or environment.

    config.txt can contain either:
      GEMINI_API_KEY=your_key_here
      GOOGLE_API_KEY=your_key_here

    You can also hardcode locally by replacing the placeholder below.
    """
    config_path = BASE_DIR / "config.txt"
    if config_path.exists():
        try:
            ...
            print(f"  Warning: could not read Gemini key from config.txt: {e}")

    HARDCODED_GEMINI_API_KEY = "PASTE_YOUR_GEMINI_API_KEY_HERE"
    if HARDCODED_GEMINI_API_KEY and HARDCODED_GEMINI_API_KEY != "PASTE_YOUR_GEMINI_API_KEY_HERE":
...
if USE_GEMINI_FOR_CAMERA_CAPTURES and not GEMINI_API_KEY:
    print("  [!] Gemini key not found. Camera captures will fall back to Claude Sonnet.")


def gemini_generate_from_blocks(image_blocks, prompt, model=None, max_tokens=3000, temperature=0.1):
    """Call Gemini through the REST API using the same image/text block format used for Claude.

    This avoids requiring the google-genai package. It uses only urllib from the standard library.
    """
    if not GEMINI_API_KEY:
        raise RuntimeError("Missing Gemini API key")

    model = model or GEMINI_CAMERA_MODEL
    parts = []
    ...
    def _do_gemini_request():
        try:
            ...
            # Surface the real error body — Gemini explains 400/403/429 in the body.
            try:
                body = http_err.read().decode("utf-8", errors="ignore")
            except Exception:
                body = ""
            raise RuntimeError(f"Gemini HTTP {http_err.code}: {body[:500]}") from http_err
        except urllib.error.URLError as url_err:
            raise RuntimeError(f"Gemini network error: {url_err}") from url_err

    data = call_with_retry(_do_gemini_request, label="Gemini")
...
try:
    name_resp = claude_messages_create_with_retry(
...
LOOKBACK_TIME_WINDOW_SECONDS = 60  # only run Smart Lookback on captures taken close together; 0 disables time filtering
...
capture_rotation_degrees = 0  # manual rotation applied to NEW camera captures/screenshots: 0, 90, 180, 270 clockwise
...
def _rotate_cv_image(img_bgr, degrees):
    """Rotate an OpenCV BGR image clockwise by 0/90/180/270 degrees."""
    deg = _normalize_rotation(degrees)
    if deg == 90:
...
def _rotate_pil_image(img, degrees):
    """Rotate a PIL image clockwise by 0/90/180/270 degrees."""
    deg = _normalize_rotation(degrees)
    if deg == 0:
        return img
    # PIL rotates counter-clockwise for positive values, so use negative for clockwise.
    return img.rotate(-deg, expand=True)


def _apply_rotation_to_saved_image(path, degrees):
    """Rotate an already-saved image file in place. Used for screenshot fallback paths."""
    deg = _normalize_rotation(degrees)
    if deg == 0:
        return
    try:
        ...
        with _PILImage.open(path) as im:
            rotated = _rotate_pil_image(im.convert("RGB"), deg)
            rotated.save(path)
    except Exception as e:
        print(f"  [!] Could not rotate saved image: {e}")


def set_capture_rotation(degrees):
    """Set manual rotation mode for future camera captures and screenshots."""
    global capture_rotation_degrees
    capture_rotation_degrees = _normalize_rotation(degrees)
    if capture_rotation_degrees == 0:
        print("  [ROTATE] Rotation reset. Future captures will be normal orientation.")
    else:
        print(f"  [ROTATE] Future captures/screenshots will rotate {capture_rotation_degrees}° clockwise before AI processing.")
screenshot_api_lock = threading.Lock()  # serializes screenshot Claude calls so screenshots queue instead of spiking rate limits
...
print("  R                     = toggle 180° rotation for future captures")
print("  T                     = rotate future captures +90° clockwise")
...
def detect_cameras():
    ...
    for i in range(6):
        ...
        if test.isOpened():
            ...
            # Opening a non-existent camera index can be slow on some systems.
            # Once we hit two misses in a row, higher indices are almost never real.
            if consecutive_misses >= 2 and found:
                break
    return found
...
def snap_photo():
    ...
    # Save as PNG, not JPEG. Math reading errors (a + read as -, a 3 read as 8)
    # almost always come from JPEG ringing artifacts around thin equation strokes.
    # PNG is lossless, so the model sees the exact pixels the camera captured.
    img_path = IMAGES_DIR / f"capture_{ts}.png"

    # Adaptive brightness — but only when the room is genuinely dark.
    # CLAHE on an already-bright whiteboard boosts local contrast unevenly and can
    # crush faint chalk/marker strokes, which HURTS reading accuracy. So we measure
    # the luminance first and only enhance dim frames.
    lab = cv2.cvtColor(frame, cv2.COLOR_BGR2LAB)
    l_ch, a_ch, b_ch = cv2.split(lab)
    mean_luma = float(l_ch.mean())
    if mean_luma < 110:
        # Dark room: gentle CLAHE genuinely helps recover the board.
        clip = 2.0 if mean_luma < 80 else 1.5
        clahe = cv2.createCLAHE(clipLimit=clip, tileGridSize=(8, 8))
        l_ch = clahe.apply(l_ch)
        enhanced = cv2.cvtColor(cv2.merge([l_ch, a_ch, b_ch]), cv2.COLOR_LAB2BGR)
        print(f"  [{ts}] Dark frame (luma={mean_luma:.0f}) — applied CLAHE clip={clip}.")
    else:
        # Bright enough already: keep the raw frame, no contrast distortion.
        enhanced = frame

    # Manual orientation fix: R/T hotkeys set this for future captures.
    if capture_rotation_degrees:
        enhanced = _rotate_cv_image(enhanced, capture_rotation_degrees)

    # PNG compression level 3 = fast write, still lossless.
    cv2.imwrite(str(img_path), enhanced, [cv2.IMWRITE_PNG_COMPRESSION, 3])
    ...
    # If the prefilter kept this photo *because* it is a sharper retake of the
    # previous board, Smart Lookback is genuinely worth running (it can replace
    # the older blurry capture). Otherwise this is a fresh board and lookback
    # would just burn a Sonnet call comparing unrelated content.
    similar_to_previous = bool(duplicate_reason and "much sharper" in duplicate_reason)

    queue_pos = capture_queue.qsize() + 1
    print(f"  [{ts}] Photo #{queue_pos + len(captures)} taken — queued for AI processing...")
    flash_frame = True
    capture_queue.put({
        "path": str(img_path),
        "timestamp": ts,
...
def _timestamp_to_seconds(ts):
    """Convert LectureLens timestamps like HH-MM-SS into seconds since midnight.

    Returns None if the timestamp cannot be parsed. This is only used for the
    Smart Lookback time window, so failing safely means "do not compare."
    """
    try:
...
def _capture_gap_seconds(newer_capture, older_capture):
    """Return newer-old capture timestamp gap in seconds, or None if unknown."""
    newer = _timestamp_to_seconds((newer_capture or {}).get("timestamp", ""))
    older = _timestamp_to_seconds((older_capture or {}).get("timestamp", ""))
    if newer is None or older is None:
        return None
    gap = newer - older
    # If a lecture crosses midnight, allow simple wraparound. Rare, but harmless.
    if gap < 0:
        gap += 24 * 3600
    return gap


def _within_lookback_time_window(newer_capture, older_capture):
    """Only compare captures taken close together.

    This prevents a later board/topic from accidentally deleting an older useful
    capture. Set LOOKBACK_TIME_WINDOW_SECONDS = 0 to disable this filter.
    """
    if not LOOKBACK_TIME_WINDOW_SECONDS:
        return True, None
    gap = _capture_gap_seconds(newer_capture, older_capture)
    if gap is None:
        return False, None
    return gap <= LOOKBACK_TIME_WINDOW_SECONDS, gap
...
def smart_repair_recent_captures(similar_to_previous=False):
    ...
    Gating: this fires a full multi-image Sonnet call, so it should NOT run on every
    capture. It runs only when there is a concrete reason:
      - similar_to_previous: the local prefilter said this is a sharper retake, or
      - the newest notes OR the immediately previous notes look unclear.
    A fresh, clearly-read board does not need a lookback call at all.
    """
    ...
    try:
        with captures_lock:
            if len(captures) < 2:
                return
            # Decide whether a lookback call is worth the cost.
            newest_unclear = _looks_unclear(captures[-1].get("notes", ""))
            prev_unclear = _looks_unclear(captures[-2].get("notes", ""))
        if not (similar_to_previous or newest_unclear or prev_unclear):
            print("  Smart lookback: skipped — fresh board, captures read cleanly.")
            return
        ...
        with captures_lock:
            ...
            # Look back only over recent non-deleted captures.
            # This saves cost and prevents a later/new topic from deleting an older useful capture.
            candidate_indices = []
            skipped_due_to_time = 0
            for idx in range(curr_index - 1, -1, -1):
                if len(candidate_indices) >= 3:
                    break
                if captures[idx].get("skip_in_final_outputs"):
                    continue

                in_window, gap = _within_lookback_time_window(curr, captures[idx])
                if not in_window:
                    skipped_due_to_time += 1
                    # Captures are chronological. Once we hit an older capture outside
                    # the window, anything before it is also outside the window.
                    break
        ...
        if not candidates and SMART_LOOKBACK_REPAIR:
            if LOOKBACK_TIME_WINDOW_SECONDS:
                print(f"  [{curr.get('timestamp','')}] Smart lookback skipped — no active captures within {LOOKBACK_TIME_WINDOW_SECONDS}s.")
            return
        ...
        print(f"  [{curr.get('timestamp','')}] Smart lookback: checking newest capture against {len(candidates)} active capture(s) within {LOOKBACK_TIME_WINDOW_SECONDS}s...")
...
- You are only being shown previous captures that were taken close in time to the newest capture. Treat these as possible retakes/nearby board states.
...
CURRENT_NOTES>>>"""
        ...
        response = claude_messages_create_with_retry(
...
# ── Camera capture prep: full image + content-aware crops for Claude ───────
# Vision models (Claude, Gemini) internally downsample to roughly a 1568px long
# edge. Sending anything larger just burns tokens/latency with zero accuracy gain,
# and upscaling a crop with LANCZOS adds no real detail — it only inflates tokens.
# So: cap everything at ~1568px, and only modestly upscale crops that are genuinely
# tiny (a small equation in a big frame), never past the model's own cap.
MODEL_VISION_LONG_EDGE = 1568   # effective resolution ceiling for the model
MIN_USEFUL_CROP_EDGE = 700      # crops smaller than this get a gentle upscale


def _prep_capture_for_claude(path):
    """Prepare a camera capture: full image for context + a few content-aware crops.

    Old behaviour sent 4 hardcoded fixed-fraction crops that heavily overlapped each
    other and the full image — same equation 4-5 times at different scales, which
    wasted tokens and could make the model treat one problem as several. This version
    sends only crops that are actually driven by where the content is.
    """
    import io as _io
    from PIL import Image as _PILImage
    import numpy as _np
    ...
    def _encode_png(im):
        """Encode at the model's effective resolution. Downscale big images;
        gently upscale only genuinely tiny crops. Never upscale past the cap."""
        im = im.convert("RGB")
        w, h = im.size
        long_edge = max(w, h)
        if long_edge > MODEL_VISION_LONG_EDGE:
            scale = MODEL_VISION_LONG_EDGE / long_edge
            im = im.resize((max(1, int(w * scale)), max(1, int(h * scale))), _PILImage.LANCZOS)
        elif long_edge < MIN_USEFUL_CROP_EDGE:
            # Small crop — a modest upscale helps the model lock onto thin strokes,
            # but only up to the model cap. Past that is pure waste.
            target = min(MIN_USEFUL_CROP_EDGE * 2, MODEL_VISION_LONG_EDGE)
    ...
    full_b64 = _encode_png(img)
    crops = []

    def _add_crop(label, box):
        ...
        if (x1 - x0) < 40 or (y1 - y0) < 30:
            return
        crops.append({"label": label, "data": _encode_png(img.crop((x0, y0, x1, y1)))})

    found_content_crop = False

    # Content-aware crop 1: bright board/page/screen region (ground truth for printed math).
    try:
        ...
        if len(rows) > 20 and len(cols) > 20:
            ...
            if (x1 - x0) * (y1 - y0) > W * H * 0.01:
                _add_crop(
                    "DETECTED LIGHT BOARD/PAPER/SCREEN - ground truth for exact math",
                    (x0 - 45, y0 - 35, x1 + 45, y1 + 35)
                )
                found_content_crop = True
    except Exception as e:
        print(f"  [prep] light-content crop skipped: {e}")

    # Content-aware crop 2: dense dark writing/text region (handwriting, signs, numbers).
    try:
        arr = _np.array(img.convert("L"))
        dark = arr < 120
        text_rows = _np.where(dark.mean(axis=1) > 0.002)[0]
        text_cols = _np.where(dark.mean(axis=0) > 0.001)[0]
        if len(text_rows) > 10 and len(text_cols) > 10:
            ...
            if (x1 - x0) > 30 and (y1 - y0) > 20:
                _add_crop(
                    "DETECTED DENSE WRITING - use for exact signs, coefficients, constants",
                    (x0 - 60, y0 - 50, x1 + 60, y1 + 50)
                )
                found_content_crop = True
    except Exception as e:
        print(f"  [prep] dense-text crop skipped: {e}")

    # Fallback: if neither detector found anything (low contrast / odd lighting),
    # send one generous center crop so the model still gets a zoomed view.
    if not found_content_crop:
        _add_crop(
            "CENTER REGION (fallback) - zoomed view for exact math",
            (int(W * 0.06), int(H * 0.05), int(W * 0.94), int(H * 0.95))
        )
...
def process_queue():
    while True:
        ...
        print(f"  [{ts}] Processing capture #{capture_num} with Gemini/Claude hybrid...")
        ...
        try:
            ...
            if crops:
                print(f"  [{ts}] Sending full camera capture + {len(crops)} content-aware crop(s).")
            else:
                print(f"  [{ts}] Sending full camera capture only.")

            # The READ pass needs every crop (exact transcription). The SOLVE pass
            # mainly works from the extracted text — it only needs the full image
            # to sanity-check, not the crops again. Sending crops twice doubles
            # image tokens for no accuracy gain, so the solve pass uses this lighter set.
            full_image_block = [{
                "type": "text",
                "text": "FULL CAMERA CAPTURE (for cross-checking the extracted text against the board)."
            }, {
                "type": "image",
                "source": {"type": "base64", "media_type": "image/png", "data": full_b64}
            }]
...
(only include conflicts, unclear entries, or inferred values; otherwise write "No reading issues.")"""
            ...
            # ── PASS 1 + PASS 2: Gemini for normal camera notes ──────────
            # This is the cost-saving hybrid:
            # - Gemini handles ordinary camera capture reading/notes.
            # - Claude Sonnet still handles Smart Lookback/delete after notes are created.
            # - Claude Sonnet also remains for screenshots and final summary elsewhere in the file.
            if USE_GEMINI_FOR_CAMERA_CAPTURES and GEMINI_API_KEY:
                try:
                    print(f"  [{ts}] Gemini Pass 1: Reading board...")
                    extracted = gemini_generate_from_blocks(
                        image_blocks,
                        READ_PROMPT,
                        model=GEMINI_CAMERA_MODEL,
                        max_tokens=3000,
                        temperature=0.05
                    )
                    print(f"  [{ts}] Gemini Pass 1 done. Gemini Pass 2: Building notes...")

                    combined_solve_prompt = (
                        "Here is the exact extraction from Pass 1:\n\n"
                        "<<<EXTRACTED_CONTENT>>>\n"
                        f"{extracted}\n"
                        "<<<END_EXTRACTED_CONTENT>>>\n\n"
                        + SOLVE_PROMPT
                    )
                    notes = gemini_generate_from_blocks(
                        full_image_block,
                        combined_solve_prompt,
                        model=GEMINI_CAMERA_MODEL,
                        max_tokens=5000,
                    ...
                    notes = notes + f"\n\n## Processing Note\nNormal camera capture processed with Gemini model `{GEMINI_CAMERA_MODEL}`. Smart Lookback/delete still uses Claude Sonnet."
                except Exception as gemini_err:
                    print(f"  [{ts}] [!] Gemini failed, falling back to Claude Sonnet: {gemini_err}")
                    print(f"  [{ts}] Claude Pass 1: Reading board...")
                    read_resp = claude_messages_create_with_retry(
                        model="claude-sonnet-4-5",
                        max_tokens=3000,
                        messages=[{
                            "role": "user",
                    ...
                    print(f"  [{ts}] Claude Pass 1 done. Claude Pass 2: Solving with full capture + crops...")
                    solve_resp = claude_messages_create_with_retry(
                        model="claude-sonnet-4-5",
                        max_tokens=6000,
                        messages=[
                            {"role": "user", "content": image_blocks + [{"type": "text", "text": READ_PROMPT}]},
                            {"role": "assistant", "content": extracted},
                            {"role": "user", "content": full_image_block + [{"type": "text", "text": SOLVE_PROMPT}]}
                        ]
                    )
                ...
                extracted = read_resp.content[0].text.strip()
                print(f"  [{ts}] Claude Pass 1 done. Claude Pass 2: Solving...")
                solve_resp = claude_messages_create_with_retry(
                    model="claude-sonnet-4-5",
                    max_tokens=6000,
                    messages=[
            ...
            # Gated — only runs when there's a real reason (retake or unclear notes).
            smart_repair_recent_captures(similar_to_previous=item.get("similar_to_previous", False))
            ...
            print(f"  [{ts}] ERROR calling AI model: {e}")
            traceback.print_exc()
        finally:
...
<div class="toc"><strong style="font-family:Arial,sans-serif;font-size:13px">Contents</strong>\n{toc}</div>\n"""
    ...
    try:
        ...
        summary_resp = claude_messages_create_with_retry(
            model="claude-sonnet-4-5",
            max_tokens=6000,
...
<div class="footer">Generated by Claude Sonnet &mdash; LectureLens &mdash; {session_name}</div>
    ...
    try:
        ...
        rename_resp = claude_messages_create_with_retry(
            model="claude-haiku-4-5-20251001",
            max_tokens=20,
...
def live_preview():
    ...
    while not stopped:
        ...
        if capture_rotation_degrees:
            cv2.putText(display, f"ROTATE: {capture_rotation_degrees} deg clockwise", (10, 88),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 200, 255), 2)
        hint = "SPACE=capture  S=screenshot  N=note  O=photo-note  R/T=rotate  TAB=cam  ESC=stop" if len(available_cameras) > 1 else "SPACE=capture  S=screenshot  N=note  O=photo-note  R/T=rotate  ESC=stop"
...
def add_text_note():
    ...
    try:
        ...
        claude_answer = ""
        try:
            note_resp = claude_messages_create_with_retry(
                model="claude-sonnet-4-5",
                max_tokens=500,
                messages=[{
                    "role": "user",
...
def add_photo_note():
    ...
    try:
        ...
        if ret:
            ...
            photo_path = IMAGES_DIR / f"note_photo_{ts_photo}.png"
            lab = cv2.cvtColor(frame, cv2.COLOR_BGR2LAB)
            l_ch, a_ch, b_ch = cv2.split(lab)
            mean_luma = float(l_ch.mean())
            if mean_luma < 110:
            ...
            if capture_rotation_degrees:
                enhanced = _rotate_cv_image(enhanced, capture_rotation_degrees)
            cv2.imwrite(str(photo_path), enhanced, [cv2.IMWRITE_PNG_COMPRESSION, 3])
...
def _prep_screenshot_for_claude(path):
    ...
    def _encode_png(im, max_long_edge=None, upscale_to=None):
        ...
        if max_long_edge and max(w, h) > max_long_edge:
            ...
            im = im.resize((max(1, int(w * scale)), max(1, int(h * scale))), _PILImage.LANCZOS)
            w, h = im.size
        # Never upscale past the model's effective resolution — LANCZOS upscaling
        # past that point adds tokens, not detail.
        if upscale_to:
            upscale_to = min(upscale_to, MODEL_VISION_LONG_EDGE)
    ...
    full_b64 = _encode_png(img, max_long_edge=MODEL_VISION_LONG_EDGE)
    ...
    def _add_crop(label, box, upscale_to=1400):
        ...
        max_edge = MODEL_VISION_LONG_EDGE if max(crop.size) > MODEL_VISION_LONG_EDGE else None
        crops.append({"label": label, "data": _encode_png(crop, max_long_edge=max_edge, upscale_to=upscale_to)})

    # WebWork/page problems sit near the top; the dark handwritten scratch canvas
    # is usually elsewhere. We keep ONE strong fixed crop (top region, where the
    # printed problem lives) instead of four overlapping ones — the old version
    # sent the same equation 4x at different scales, wasting tokens. The two
    # content-aware crops below do the precise zooming.
    _add_crop(
        "TOP OF SCREEN - printed WebWork/problem statement (NOT the dark scratch canvas)",
        (0, 0, int(W * 0.95), int(H * 0.50)), upscale_to=1568
    )
...
def screenshot_and_note():
    ...
    try:
        ...
        try:
            ...
            img = ImageGrab.grab()
            if capture_rotation_degrees:
                img = _rotate_pil_image(img, capture_rotation_degrees)
            ...
            if capture_rotation_degrees:
                _apply_rotation_to_saved_image(str(screenshot_path), capture_rotation_degrees)
        ...
        print(f"  [{ts}] Screenshot queued for Claude processing...")

        # Only one screenshot Claude job runs at a time. This acts like a screenshot queue:
        # no delay when there is no backlog, but extra screenshots wait here instead of
        # all hitting the API/rate limit at once.
        with screenshot_api_lock:
    ...
    Return the complete corrected markdown notes only. No preamble."""
                    audit_resp = claude_messages_create_with_retry(
                        model=MODEL_SS,
                        max_tokens=6000,
...
def on_press(key):
    ...
    try:
        ...
        # N key — add text note in terminal
            ...
            # Toggle 180° rotation for future captures/screenshots.
            # Press R again to return to normal.
            set_capture_rotation(0 if capture_rotation_degrees == 180 else 180)
        elif hasattr(key, 'char') and key.char and key.char.lower() == 't':
            # Rotate future captures/screenshots +90° clockwise each press.
            # Cycle: 0 → 90 → 180 → 270 → 0.
            set_capture_rotation(capture_rotation_degrees + 90)
    except Exception:
        pass
